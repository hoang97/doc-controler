...
$(document).ready(function () {
    var xfileStatusId=$('#xfileStatusId').text();
    getXfiles(xfileStatusId,true);
});


function getXfiles(xfileStatusId,initial=false){
    $.ajax({
        type: 'GET',
        url: '/get-xfiles-by-status',
        data: {
         'status':xfileStatusId,
        },
        ...
        // ]
                ...
                for (let i in data){
                            count+=1;
                            displayTableList.push(dictXfileToList(data[i],count));
                }
...
function initXfileTable(dataSet,initial=false){
       ...
       if (initial==true){
        ...
        if (dataSet.length>0){
            xfileTableElement.dataTable().fnAddData(dataSet);
        }
    }
}
...
function dictXfileToList(dict,count){
    ...
    if (requestEditorUser==true && dict['status']==1){
    btnXoa='<button type="button" class="btn btn-danger float-right" data-toggle="modal" data-target="#modalDelete" id="rowDelete-'+dict['id']+'" data-id="'+dict['id']+'" data-typestatus="'+dict['status']+'"> <span data-toggle="tooltip" data-placement="bottom" title="Xoá bản ghi"<i class="fas fa-trash-alt"></i></span></button>'
}
    ...
    let btnView='<a  class="btn btn-info float-right" href="'+hrefXfile+'"> <span data-toggle="tooltip" data-placement="bottom" title="Xem HSMT"><i class="fas fa-folder-open"></i></span></a>'
    if(dict['ct']){
        arr.push(btnView);
    }
    else{    arr.push(btnXoa+btnDetail+btnDuplicate);
    }
    return arr;
}   

function DeleteXfile(XfileId,xfileStatusId){
    let data={
        'id':XfileId
    }
    $.ajax({
...
$('#modalDelete').on('show.bs.modal', function (event) {
    ...
    let deleteQuery='DeleteXfile("'+XfileId+'","' +xfileStatusId+'");';
